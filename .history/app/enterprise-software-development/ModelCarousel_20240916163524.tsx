      ...
      <CarouselContent>
        {items.map((item, index) => (
          <CarouselItem key={index}>
            <div className="p-1">
              <div className=" md:px-0 py-5 md:py-10 px-10 flex flex-col md:flex-row items-start justify-center md:justify-between">
                ...
                <div className="md:w-[70%] flex flex-col gap-3 md:gap-10 items-start justify-center"><span className="text-xl md:text-2xl font-semibold text-slate-800">{item.title}</span>
                <p className="text-slate-800 md:pr-20 md:text-lg text-sm text-justify">{item.description}</p></div>
              </div>
            </div>
          </CarouselItem>
        ))}
