      ...
      <CarouselContent>
        {items.map((item, index) => (
          <CarouselItem key={index}>
            <div className="p-1">
              <div className=" px-0 py-10 flex flex-col md:flex-row items-start justify-between">
                <div className="md:w-[30%]"><h3 className=" font-semibold md:px-10 text-xl md:text-4xl text-slate-800">{item.precursor}</h3></div>
                <div><Separator orientation="vertical"/></div>
                <div className="md:w-[70%] flex flex-col gap-10 items-start justify-center"><span className="text-2xl font-semibold text-slate-800">{item.title}</span>
