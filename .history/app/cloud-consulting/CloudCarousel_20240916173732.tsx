      ...
      <CarouselContent>
        {items.map((item, index) => (
          <CarouselItem key={index}>
            <div className=" p-1">
            <div className="bg-red-500  md:px-0 py-5 md:py-10 px-5 flex flex-col md:flex-row items-start justify-center md:justify-between">
            <div className=" md:w-[30%]">
                  <h3 className=" font-semibold md:px-10 text-2xl md:text-4xl text-slate-800">{item.precursor}</h3>
                </div>
                <div><Separator orientation="vertical" /></div>
                <div className="md:w-[70%] flex flex-col gap-3 md:gap-10 items-start justify-center">
                  <span className="text-xl md:text-2xl font-semibold text-slate-800">{item.title}</span>
                  <p className="text-slate-800 md:pr-20 md:text-lg text-sm text-justify"><ClientSideHtml html={item.description} /></p>
                </div>
              </div>
            </div>
          </CarouselItem>
