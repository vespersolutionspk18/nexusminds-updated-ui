        ...
        {items.map((item, index) => (
          <CarouselItem key={index}>
            ...
            <div className=" md:w-[30%]">
                ...
                <div className="md:w-[70%] w-full flex flex-col gap-3 md:gap-10 items-start justify-center">
                  <span className="text-xl md:text-2xl font-semibold text-slate-800">{item.title}</span>
                  <p className="w-full text-slate-800 pr-0 md:pr-20 md:text-lg text-sm "><ClientSideHtml html={item.description} /></p>
                </div>
              </div>
            </div>
          </CarouselItem>
...
function ClientSideHtml({ html }: { html: string }) {
  ...
  return <p className="text-slate-800 md:pr-20 text-sm md:text-lg" dangerouslySetInnerHTML={{ __html: content }} />;
}
